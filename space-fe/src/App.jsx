...
// Camera zoom that keeps every live shot in view, easing back to the default
// framing once they're gone. Shots can leave the screen and be pulled back by
// gravity, so the camera zooms out to follow them.
const VIEW_PADDING = 40; // world-space breathing room to leave around shots
const ZOOM_SMOOTHNESS = 6; // eases the camera toward its target; higher = faster
const MIN_ZOOM = 0.4; // never shrink the view past this, even for far-flung shots
...
function drawScene(ctx, planets, shots, explosions, charge, camera) {
  ...
  // HUD is drawn in screen space so it stays pinned to the corners regardless
  // of zoom. Remaining lifetime of each shot, stacked from the bottom-right.
  ctx.fillStyle = "#ffd94a";
...
function App() {
  ...
  // Camera used to zoom out and keep every live shot framed. `scale` of 1 with
  ...
  useEffect(() => {
    ...
    const tick = (time) => {
      ...
      // Fit the camera to a box holding the default frame plus every live shot
      // (padded). Unioning with the frame means we only ever zoom out, never
      // in, so with no shots the box is exactly the frame and the view eases
      // back to default. Clamp the zoom-out so a far-flung shot can't shrink
      // the game to a speck.
      let minX = 0;
      let minY = 0;
      let maxX = WIDTH;
      ...
      for (const shot of shotsRef.current) {
        minX = Math.min(minX, shot.x - VIEW_PADDING);
        maxX = Math.max(maxX, shot.x + VIEW_PADDING);
        minY = Math.min(minY, shot.y - VIEW_PADDING);
        maxY = Math.max(maxY, shot.y + VIEW_PADDING);
      }
